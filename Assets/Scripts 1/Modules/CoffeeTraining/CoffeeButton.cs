...
public class CoffeeButton : MonoBehaviour
{
    ...
    private void Update()
    {
        if (UserSession.Instance.Role != "trainee") return;
        if (TouchOrClickDown())
        {
    ...
    public void OnClick()
    {
        ...
        if (UserSession.Instance.Role == "trainee")
        {
            FirebaseFirestore db = FirebaseFirestore.DefaultInstance;
            string docId = UserSession.Instance.UserId + "_session";
            Dictionary<string, object> update = new Dictionary<string, object>
            {
                { "lastButton", buttonType.ToString() },
                { "lastButtonTimestamp", Timestamp.GetCurrentTimestamp() }
            };

            db.Collection("liveSessions").Document(docId)
